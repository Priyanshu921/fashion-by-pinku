...
const Login = () => {
  ...
  useEffect(() => {
    document.title = "Sign In | FASHION BY PINKU";
  }, []);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    import('goey-toast').then(({ goeyToast }) => {
      const authPromise = axios.post('/api/auth/login', { email, password }).then((response) => {
      ...
      goeyToast.promise(authPromise, {
        ...
        description: {
          error: (err: any) => err?.response?.data?.message || err?.message || 'Failed to login',
        }
      });
        ...
        <div className="max-w-md w-full space-y-8 bg-white/5 p-10 rounded-2xl shadow-2xl border border-white/10 backdrop-blur-md">
          ...
          <form className="mt-8 space-y-6" onSubmit={handleLogin}>
            ...
            <p className="text-center text-white/50 text-sm mt-4">
              Don't have an account? <Link to="/register" className="text-brand-pink hover:text-white transition-colors">Register here</Link>
            </p>
          </form>
        </div>
      </ScrollFadeIn>
